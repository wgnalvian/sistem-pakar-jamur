...
@section('content')
    <div class="row tw-px-10 tw-box-border">
        <div class="col-12 tw-px-4 mt-7 tw-box-border">
           <h6 class="font-weight-bolder tw-text-4xl  text-white mb-5">{{$title}}</h6> 
        </div>
    </div>
        ...
        <div class="col-12 ">
            <div class="card mb-4">
                ...
                <div class="card-body p-0 px-4 pb-2 row" >
                    <div class="col-1">
                        <img src="/images/{{$isharvestConditions[0]->isharvest->image}}" alt="" class="tw-w-20 tw-h-20">
                    </div>
                    <div class="col-10 tw-flex tw-flex-col tw-items-start tw-justify-start tw-align-top  ">
                        <div class="tw-flex tw-text-sm">
                            ...
                            <p class="tw-text-green-500 tw-font-bold">{{$isharvestConditions[0]->isharvest->isharvest}}</p>
                        </div>
                        <div class="tw-flex">
                            <p class=" tw-font-bold">Kode</p>
                            <p class=" tw-font-bold tw-ml-6 tw-mr-3">:</p>
                            <p class="tw-text-green-500 tw-font-bold">{{$isharvestConditions[0]->isharvest->code}}</p>
                        </div>
                    </div>
    ...
    <div class="row mx-3">
        <div class="card">
            ...
            <div class="card-body">
                <p>{{$isharvestConditions[0]->isharvest->description}}</p>
            </div>
        </div>
    ...
    <div class="row mx-3 mt-3">
        <div class="card">
            ...
            <div class="card-body">
                <p>{{$isharvestConditions[0]->isharvest->treatment}}</p>
            </div>
        </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body box-border px-4 pt-0 pb-2 mb-4">
                    <table class="tw-table-auto tw-w-full tw-border">
                        ...
                        <tbody>
                          <?php $i = 1;?>
                            @foreach ($isharvestConditions as $isharvestCondition)
                           
                                <tr class=" tw-border-b">
                                    <td class=" tw-border-r tw-text-sm tw-box-border tw-px-3">{{$i}}</td>
                                    <td class="tw-text-sm tw-box-border tw-border-r tw-px-3">{{ $isharvestCondition->condition->name }}</td>
                                    <td class="tw-text-sm tw-box-border tw-border-r tw-px-3">{{ $isharvestCondition->condition->code }}</td>
                                    <td class="tw-text-sm tw-box-border tw-border-r tw-px-3">{{ $isharvestCondition->cf }}</td>
                                       @if(session()->get('user')->name == "Admin")
                                    <td class="tw-text-sm tw-box-border tw-px-3 tw-flex tw-flex-col tw-border-r tw-align-middle tw-items-start tw-justify-start ">
                                        <button data-bs-toggle="modal" data-bs-target="#deleteDesease{{$isharvestCondition->id}}"  class="btn btn-warning btn-sm tw-mt-3"> <i
                                ...
                                <!-- Modal Hapus -->
                                <div class="modal" tabindex="-1" id="deleteDesease{{$isharvestCondition->id}}">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                              <p>Apakah kamu yakin ingin menghapus gejala {{$isharvestCondition->condition->code}} ?</p>
                                            </div>
                                            <div class="modal-footer">
                                           
                                                <button type="button" class="btn btn-secondary"
                                                    data-bs-dismiss="modal">Close</button>
                                                    <form action="/isharvestCondition/detail" method="post">
                                                    @csrf
                                                    @method('delete')
                                                    <input type="hidden" name="id" value="{{$isharvestCondition->id}}">
                        ...
                        @if (count($isharvestConditions) == 0)
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form action="/isharvestCondition/detail" method="post">
                @csrf
                <input type="hidden" name="desease_id" value="{{$isharvestConditions[0]->isharvest->id}}">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" aria-label="Default select example" name="symtom_id">
                             ...
                             <?php $isExist = false; ?>
                                @foreach ($isharvestConditions as $isharvestCondition)
                                    @if ($condition->id == $isharvestCondition->condition->id)
                                        <?php $isExist = true; ?>
                                    @endif
                                @endforeach
                                 @if ($isExist == false)
