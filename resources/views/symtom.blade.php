                    ...
                    <table class="tw-table-auto tw-w-full ">
                        <thead>
                            <tr style="background-image: linear-gradient( 90.9deg,  rgba(3,195,195,1) 0.3%, rgba(37,84,112,1) 87.8% );" class=" tw-border-b tw-rounded-t-md">
                                ...
                                <th class="tw-w-2/6 tw-text-white  tw-border-r tw-py-2 tw-box-border tw-px-3 ">Gejala</th>
                          ...
                          <?php $i = 1;?>
                            @foreach ($symtoms as $symtom)
                                <tr class=" tw-border-b">
                                    ...
                                    <td class="tw-box-border tw-border-r tw-pxP-3">{{ $symtom->code }}</td>
                                    <td class="tw-box-border tw-border-r tw-px-3">{{ $symtom->name }}</td>
                                     @if(session()->get('user')->name == "Admin")
                                <td class="tw-box-border tw-px-3 tw-flex tw-flex-col tw-border-r tw-align-middle tw-items-start tw-justify-start ">
                                        <button data-bs-toggle="modal" data-bs-target="#deleteSymtom{{$symtom->id}}"  class="btn btn-warning btn-sm tw-mt-3"> <i
                                                class=" tw-mr-2 fa fa-trash" aria-hidden="true"></i> Hapus </button>
                                        <button data-bs-toggle="modal" data-bs-target="#editSymtom{{$symtom->id}}" class="btn btn-danger btn-sm"><i
                                ...
                                {{-- Modal Edit --}}
                                <div class="modal fade" id="editSymtom{{$symtom->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                  <div class="modal-dialog">
                                      <form action="/symtom" method="post">
                                          @csrf
                                          @method('patch')
                                          <input type="hidden" name="id" value="{{$symtom->id}}">
                                          <div class="modal-content">
                                              ...
                                              <div class="modal-body">
                                                  <div class="mb-3">
                                                      ...
                                                      <input type="text" class="form-control" name="name" value="{{$symtom->name}}">
                                                  </div>
                                                  @error('name')
                                                      <div class="alert alert-danger text-white">{{ $message }}</div>
                                                  @enderror
                                                  <div class="mb-3">
                                                      <label for="exampleInputPassword1" class="form-label">Kode Gejala</label>
                                                      <input type="text" value="{{$symtom->code}}"  class="form-control" name="code">
                                                  </div>
                                                  @error('code')
                                                      <div class="alert alert-danger text-white">{{ $message }}</div>
                                                  @enderror
                                ...
                                <!-- Modal Hapus -->
                                <div class="modal" tabindex="-1" id="deleteSymtom{{$symtom->id}}">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            ...
                                            <div class="modal-body">
                                              <p>Apakah kamu yakin ingin menghapus gejala {{$symtom->code}} ?</p>
                                            </div>
                                            <div class="modal-footer">
                        ...
                        @if (count($symtoms) == 0)
                            <div class="alert alert-danger text-white">Data Gejala Kosong</div>
